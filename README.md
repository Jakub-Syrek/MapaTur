...
[![Tests](https://img.shields.io/badge/tests-1753%20passing-brightgreen)](#testing)
...
| **Walk mode (F8) — first-person hiking** | ✅ Verified | `WalkPhysics` in real metres (gravity, jump + double-jump, slope gate, ciupaga self-arrest on steep rock); procedural first-person ciupaga viewmodel; walk owns the camera near-plane (0.3 m) so the ground never clips underfoot |
| **Dragon flight (F7) + AI flock + fire combat** | ✅ Verified | Rigged glTF dragons (CPU skinning via SharpGLTF, procedural or baked animation clips): banked coordinated turns with expo + critically-damped precision steering, summit landing cycle (foot-anchored perch on the *rendered* rock), 3 autonomous AI dragons circling the peaks, homing fireballs with a ±10° lock cone — a hit kills |
| **Hyper-real volumetric fire (desktop)** | ✅ Verified | HDR RGBA16F chain + ACES; physical blackbody colour (Planckian-locus fit, T^3.5 radiance); per-billboard **world-space volumetric raymarch** (velocity-aligned ellipsoids sharing one turbulent density field → the jet fuses into a column) + curl advection; soft particles against rock & dragon; ≤8 dynamic fire lights (terrain, snow, water reflection, dragon bodies, smoke); heat-haze refraction; session-persistent scorch craters |
| **Dragon audio (desktop)** | ✅ Verified | Recorded roar/growls/fire-breath/wing samples layered over procedural synthesis (which stays as the fallback); wing whoosh synced to the **posed wing-tip bone's** velocity; a flight bed of speed-wind / wing-flutter / ground-rush loops ridden by the flight state; WinRT MediaPlayer voice banks |
| **Off-trail GPX/TCX tracks (pozaszlaki)** | ✅ Verified | Import off-trail tracks into their own panel; drawn as dedicated depth-tested 3D ribbons and routable alongside the marked-trail network |
...
- **Dragon flight + hyper-real volumetric fire (desktop)**: rigged glTF dragons CPU-skinned at runtime (`SkinnedModel`, SharpGLTF) over a pure, unit-tested arcade flight body (`DragonFlight` — coordinated banked turns via `tan(bank)/V`, expo-shaped commands tracked by a critically damped spring, an autopilot summit-landing cycle that anchors the posed FEET on the rendered rock). The breath is a **per-pixel volumetric raymarch**: every fragment intersects a velocity-aligned ellipsoid and integrates emission–absorption through a **shared world-space fbm3 density field** with curl-swirl advection — neighbouring fireballs read the same medium and fuse into one turbulent column. Colour is **physical blackbody** (Planckian-locus chromaticity split from ~T^3.5 radiance) on an **HDR RGBA16F chain** with ACES; the fire is also a *light source* (≤8 greedily-merged point lights on the terrain, snow, water reflection, dragon bodies and smoke), fades softly into rock and scales (soft particles), shimmers the air above it (depth-gated heat-haze refraction) and chars session-persistent scorch craters into the slope. Sound: recorded roar/growl/fire/wing samples over procedural synthesis, wing whooshes cued by the posed wing-tip bone, and a flight bed (speed wind / wing flutter / ground rush) ridden by the flight state.
- **Walk mode (F8)**: first-person hiking on the live 1 m terrain — `WalkPhysics` (pure + tested) with gravity, jumping, a slope gate and the ciupaga self-arrest; a procedural Skia ciupaga viewmodel bobs with the stride.
...
tests/                      1750+ unit + integration tests (xUnit + FluentAssertions + FsCheck)
...
| `MapaTur.Application.Tests` | 1438 | Overpass queries (trails/POI/roads), 3D terrain math + camera + atmosphere, **baked-tile streaming SYSTEM INVARIANTS** (quadtree selector, residency planner, still-camera convergence incl. under the budget clamp, stale eviction, parallel builds, byte/count caps), ortho distance tiers + downsampler + tier scheduler, **camera anti-tunnelling** (fine-surface sampler + floor probes), DEM repair (pit despike / hole fill / dropout strips), lake seating + OSM lake-gazetteer invariants, POI merge/dedup + peak apex refinement, multi-stop route planner + use cases, overlay densification + 1 m seating, contour extraction + route-stops persistence, **walk physics + dragon flight** (landing cycle, flap boost, precision steering: shaped commands, critically damped bank, stroke gate) + AI-flock pilot + glTF skinning engine |
| `MapaTur.Infrastructure.Tests` | 142 | TCX/Overpass/POI/road parsers, MBTiles + DEM readers, GUGiK WCS tile source + cache, SQLite (trails/climbing/POI), GPX |
| `MapaTur.Routing.Tests` | 29 | Tobler function, distance/time cost functions, graph snapping, A\* correctness |
| **Total** | **1753** | xUnit + FluentAssertions + NSubstitute + FsCheck |

## Roadmap

Milestones tracked in [`docs/ROADMAP.md`](docs/ROADMAP.md). Initial milestones (M0–M6), hillshade (M7), climbing POIs (M8), the **3D terrain GPU engine (M9)**, the **baked-tile streaming re-architecture** (immutable z13–z16 pyramid + two-focus quadtree LOD + surface-ownership masking), **Slovak-side 1 m detail (DMR 5.0)**, the **GUGiK + ZBGIS cross-border ortho hybrid**, the **OSM lake gazetteer (water on all 171 tarns)**, the **async startup pipeline** (instant scene reveal), **golden-hour + night-sky atmospherics**, **multi-stop tourist navigation**, the **all-packages Windows installer**, **walk mode (F8)** and the **dragon-flight epic (F7)** — AI flock, fire combat, the hyper-real volumetric-fire pipeline (HDR → blackbody → soft particles → dynamic fire lights → world-space raymarch → heat haze → scorch), precision steering and the sample-layered audio — are complete and verified live on real Tatra data (Samsung S25 Ultra + Windows desktop). Active line of work: mobile (GLES/Adreno) verification of the latest shader work, menu regrouping, elevation-aware routing, and signed store builds.

The session-by-session engineering log lives in `docs/HANDOFF-*.md` (latest: [`docs/HANDOFF-2026-07-10-dragon-epic-complete.md`](docs/HANDOFF-2026-07-10-dragon-epic-complete.md)); terrain-pipeline rules in [`docs/TERRAIN-GRAPHICS-CHECKLIST.md`](docs/TERRAIN-GRAPHICS-CHECKLIST.md) and [`docs/TILE-PRODUCTION.md`](docs/TILE-PRODUCTION.md).
